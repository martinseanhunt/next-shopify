...
// This is a quick and diry fix for the fact that the next team don't seem to 
// understand how useMemo works, the cache was being rebuilt on every route change
// so you would see flashes of stale data when data has been overwritten in the cache at
// some later point in the user journey. 

// This is becuse useMemo only does a shallow comparison. Doing a deep compare would 
// not be very performant. This is a not very well thought out but working solution
...
// Custom function to be used with getStaticProps or getServerSideProps
// carries out a gql query server side and initializes the apollo client
// so we have the data cached and ready to go
export const initWithQuery = (query, variables) => async () => {
